...
class GaLoreProjector:
    def __init__(self, rank, verbose=False, update_proj_gap=200, scale=1.0, proj_type='std', 
                 null_space_projector=None, projection_matrix=None):
        self.rank = rank
        ...
        self.null_space_projector = null_space_projector
        self.projection_matrix = projection_matrix

    def project(self, full_rank_grad, iter):
        # Step 1: Apply null space projection first (if available)
        if self.projection_matrix is not None and self.null_space_projector is not None:
            full_rank_grad = self.null_space_projector.project_to_null_space(
                full_rank_grad, self.projection_matrix
            )
        
        # Step 2: Apply GaLore projection
